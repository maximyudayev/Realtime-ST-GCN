...
def run_eval(split):
    ...
    for vid in list_of_videos:
        ...
        for s in range(len(overlap)):
            ...
            f1 = 2.0 * (precision * recall) / (precision + recall + 0.000000001)
    ...
    print('\n')
    outputs = np.column_stack((np.asarray(f1s), np.asarray(accs)))
    print(np.mean(accs), np.mean(f1s))
    #np.savetxt('log_tcn.csv', outputs, delimiter=',')
